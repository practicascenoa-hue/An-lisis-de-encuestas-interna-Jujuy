...
if df_raw is not None:
    ...
    with tab1:
        if len(df_mes) > 0:
            ...
            if st.session_state.f_tipo:
                ...
                st.dataframe(df_f[[col_cliente, col_asesor, col_nps_puntaje, col_nps_comentario]].fillna("Sin comentario"), use_container_width=True, hide_index=True)
    ...
    # --- TAB 4: RECLAMOS (CORREGIDO INTEGRAL) ---
    with tab4:
        ...
        if len(df_mes) > 0:
            def clasificar_intencion(row):
                ...
                # Lista ampliada de dolores (Servicio + Técnico)
                dolores = ["mejorar", "sala", "espera", "demora", "tardó", "baño", "baños", "falta", "anticipado", "diferencia", "color", "revisar", "alineado", "sucio", "lavado", "cafe", "café", "atencion", "atención"]
            ...
            with col_izq:
                ...
                # CONFIGURACIÓN INTEGRAL DE CATEGORÍAS
                temas_cfg = {
                    "Plazos y Tiempos": (["demora", "tardó", "días", "espera", "fecha"], "Demoras en entrega o tiempo excesivo de espera."),
                    "Infraestructura": (["sala", "baño", "café", "asiento", "comodidad"], "Estado de las instalaciones, limpieza de baños o sala de espera."),
                    "Atención/Servicio": (["atencion", "atención", "trato", "explicación", "amabilidad"], "Calidad del trato recibido o falta de información."),
                    "Calidad Técnica": (["color", "brillo", "alineado", "ruido", "pintura", "lavado", "sucio"], "Problemas con el trabajo realizado o limpieza del vehículo.")
                }
 ...
 # --- TAB 4: RECLAMOS (LÓGICA NPS ESTRICTA + CATEGORÍAS COMPLETAS) ---
    with tab4:
        st.header("⚠️ Análisis de Reclamos y Oportunidades")
        if len(df_mes) > 0:
            def clasificar_intencion(row):
                nota, texto = row[col_nps_puntaje], str(row[col_t_concatenado]).lower()
                dolores = ["mejorar", "sala", "espera", "demora", "tardó", "baño", "baños", "falta", "color", "revisar", "alineado", "sucio", "lavado", "atencion", "atención", "trato"]
                if nota <= 6: return "⚠️ RECLAMO CRÍTICO"
                elif nota >= 9:
                    if any(d in texto for d in dolores): return "💡 OPORTUNIDAD DE MEJORA"
                    return "✅ CONFORME"
            ...
            with col_izq:
                ...
                temas_cfg = {
                    "Plazos y Tiempos": (["demora", "tardó", "fecha", "espera"], "Retrasos en la entrega o recepción."),
                    "Infraestructura": (["sala", "baño", "café", "comodidad"], "Estado de las instalaciones."),
                    "Atención/Servicio": (["atencion", "atención", "trato", "explicación"], "Calidad del servicio al cliente."),
                    "Calidad Técnica": (["color", "alineado", "ruido", "pintura", "sucio"], "Trabajo en taller o limpieza.")
                }
                ...
                if filas_b:
                    df_b = pd.DataFrame(filas_b).groupby(['Tema', 'Tipo', 'Detalle']).size().reset_index(name='Casos')
                    ...
                    fig_b.update_layout(showlegend=True, height=350, margin=dict(t=10,b=10,l=0,r=10), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
                    fig_b.update_traces(hovertemplate="<b>%{y}</b><br>Casos: %{x}<br>%{customdata[0]}<extra></extra>", width=0.5)
                    st.plotly_chart(fig_b, use_container_width=True)
