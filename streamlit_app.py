...
if df_raw is not None:
    ...
    with tab1:
        if len(df_mes) > 0:
            ...
            def crear_gauge(valor, titulo):
                fig = go.Figure(go.Indicator(mode="gauge+number", value=valor,
            ...
            with st.expander(f"💬 Comentarios de {mes_sel_nombre}"):
    ...
    # --- TAB 4: RECLAMOS (LOGICA AVANZADA DE LIMPIEZA) ---
    with tab4:
        ...
        if len(df_mes) > 0:
            def clasificar_intencion(row):
                ...
                # Borramos patrones del sistema, números y guiones para ver si queda algo humano
                ...
                if nota <= 6: return "⚠️ Reclamo Crítico"
                    ...
                    return "💡 OPORTUNIDAD DE MEJORA" if len(limpio) > 2 else "✅ CONFORME"
                return "Neutral"
            ...
            with col_izq:
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("🟢 PROMOTORES", key="t4_p_final", type="primary" if st.session_state.tab4_filter == "Promotor" else "secondary"):
                        st.session_state.tab4_filter = "Promotor"; st.rerun()
                    st.metric("", cp)
                with c2:
                    if st.button("🔴 RECLAMOS", key="t4_r_final", type="primary" if st.session_state.tab4_filter == "Reclamo" else "secondary"):
                        st.session_state.tab4_filter = "Reclamo"; st.rerun()
                    st.metric("", cr)
                if st.session_state.tab4_filter is not None:
                    if st.button("🔄 Ver Todo", key="btn_t4_clear"):
                        st.session_state.tab4_filter = None; st.rerun()
                st.write("---")
                df_p = df_mes[df_mes['Grupo_Grafico'] != "Neutral"]
                if not df_p.empty:
                    fig_t = px.pie(df_p, names='Grupo_Grafico', hole=0.5, color='Grupo_Grafico', color_discrete_map={"Reclamos": "#dc3545", "Promotores": "#198754"}, title="Distribución de Clientes")
