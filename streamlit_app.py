       ...
       st.header("⚠️ Análisis de Reclamos vs. Promotores")
        if len(df_mes) > 0:
           ...
           df_mes['Grupo'] = df_mes['Intención'].apply(lambda x: "Reclamos" if "RECLAMO" in x else ("Promotores" if x != "Neutral" else "Neutral"))
                ...
                for nom, (keys, defb) in temas_cfg.items():
                    for idx, row in df_mes[df_mes['Intención'] != "Neutral"].iterrows():
                        if any(p in str(row[col_t_concatenado]).lower() for p in keys):
                            t = "Reclamo" if "Reclamo" in row['Intención'] else "Oportunidad"
                            filas.append({"Tema": nom, "Tipo": t, "Detalle": f"<b>{t}</b>: {defb}"})
                if filas:
            ...
            with col_der:
                df_t = df_mes[df_mes['Grupo'] == "Promotores"] if st.session_state.tab4_filter == "Promotor" else (df_mes[df_mes['Grupo'] == "Reclamos"] if st.session_state.tab4_filter == "Reclamo" else df_mes
