...
st.markdown("""
    ...
    /* Color de resaltado para botones seleccionados (Azul Profesional) */
    button[kind="primary"] {
...
if df_raw is not None:
    ...
    with tab3:
        ...
        df_anio[col_csi_final] = df_anio[col_csi_final].apply(clean_val); df_anio[col_nps_puntaje] = df_anio[col_nps_puntaje].apply(clean_val)
    ...
    # --- TAB 4: RECLAMOS VS SUGERENCIAS (DISEÑO OPTIMIZADO) ---
    with tab4:
        ...
        if len(df_mes) > 0:
            def clasificar_intencion(row):
                ...
                elif nota >= 9 and len(texto) > 15: return "💡 Oportunidad de Mejora"
                elif nota >= 9: return "✅ Promotor Silencioso"
                return "Neutral"
            df_mes['Intención'] = df_mes.apply(clasificar_intencion, axis=1)
            cp, cr = len(df_mes[df_mes['Intención'].str.contains("Promotor|Sugerencia")]), len(df_mes[df_mes['Intención'] == "⚠️ Reclamo Crítico"])
            ...
            with col_izq:
                ...
                st.write("---")
                df_p = df_mes[df_mes['Intención'] != "Neutral"]
                fig_t = px.pie(df_p, names='Intención', hole=0.5, color='Intención', color_discrete_map={"⚠️ Reclamo Crítico": "#dc3545", "💡 Sugerencia de Mejora": "#0dcaf0", "✅ Promotor Silencioso": "#198754"})
            ...
            with col_der:
                if st.session_state.tab4_filter == "Promotor": df_t = df_mes[df_mes['Intención'].str.contains("Promotor|Sugerencia")]
                ...
                st.dataframe(df_t[cols_final].rename(columns={col_nps_puntaje: "Puntaje Rec.", col_t_concatenado: "Comentario Completo (Col T)"}), use_container_width=True, hide_index=True, height=550)
else:
