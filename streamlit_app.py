...
st.markdown("""
    ...
    /* Estilo por posición: 1 verde, 2 amarillo, 3 rojo */
    div.stButton:nth-of-type(1) > button { background-color: #28a745; }
    div.stButton:nth-of-type(2) > button { background-color: #ffc107; color: black; }
    div.stButton:nth-of-type(3) > button { background-color: #dc3545; }
    </style>
...
if df_raw is not None:
    ...
    if col_nps_preg and len(df) > 0:
        ...
        with c1:
            ...
            # --- BOTONES MODERNOS ---
            st.write("### Seleccione para ver detalle:")
            if "filtro" not in st.session_state: st.session_state.filtro = None
            
            b1, b2, b3 = st.columns(3)
            if b1.button(f"PROMOTORES\n{len(prom)}"): st.session_state.filtro = "Promotor"
            if b2.button(f"PASIVOS\n{len(pas)}"): st.session_state.filtro = "Pasivo"
            if b3.button(f"DETRACTORES\n{len(det)}"): st.session_state.filtro = "Detractor"
        ...
        if st.session_state.filtro:
            df_v = df[df['Segmento'] == st.session_state.filtro]
            cols_v = [c for c in [col_cliente, col_asesor, col_nps_preg, col_coment] if c]
            st.subheader(f"Listado de {st.session_state.filtro}s")
            st.dataframe(df_v[cols_v], use_container_width=True)
        else:
            st.info("Haga clic en los botones de colores para auditar los comentarios.")

    else: st.warning("No hay datos para este mes.")
