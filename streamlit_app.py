...
if "tab4_filter" not in st.session_state: st.session_state.tab4_filter = "Reclamo"
    ...
    with tab4:
        ...
        if len(df_mes) > 0:
            ...
            with c_btn1:
                ...
                is_active = st.session_state.tab4_filter == "Promotor"
            ...
            with c_btn2:
                ...
                is_active = st.session_state.tab4_filter == "Reclamo"
            ...
            st.subheader(f"Detalle de clientes: {st.session_state.tab4_filter}es")
            df_filtrado = df_mes[df_mes['Segmento_NPS'] == st.session_state.tab4_filter]
            cols_mostrar = [col_cliente, col_asesor, col_nps_puntaje, col_t_concatenado]
            st.dataframe(df_filtrado[cols_mostrar].rename(columns={col_t_concatenado: "Comentario / Concatenado (Col T)"}), 
                         use_container_width=True, hide_index=True)
        else:
            st.warning("No hay datos suficientes para generar el análisis.")
else:
