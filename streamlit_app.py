...
# Carga de Datos inicial para obtener fechas
...
if df_raw is not None:
    ...
    anio_sel = st.sidebar.selectbox("Seleccione Año", anios_disponibles)
    
    # Filtrar meses disponibles para ese año
    meses_disponibles_num = sorted(df_raw[df_raw['Año'] == anio_sel]['Mes_Num'].unique().tolist())
    meses_nombres = [meses_dict[m] for m in meses_disponibles_num]
    mes_sel_nombre = st.sidebar.selectbox("Seleccione Mes", meses_nombres)
    ...
    if col_nps and len(df) > 0:
        ...
        st.warning(f"No hay datos para {mes_sel_nombre} {anio_sel}")
else:
