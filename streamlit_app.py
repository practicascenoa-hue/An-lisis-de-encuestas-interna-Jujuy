...
if df_raw is not None:
    # --- MAPEADO Y LIMPIEZA (Base 247 líneas) ---
    ...
    mes_sel_nombre = st.sidebar.selectbox("Mes", [meses_dict[m] for m in sorted(df_anio['Mes_Num'].dropna().unique().astype(int))])
    ...
    with tab1:
        if len(df_mes) > 0:
            ...
            with c2:
                ...
                if b4.button(f"🟢 {exc_c} Exc", key="btn4"): st.session_state.update({"f_tipo":"CSI","f_val":"Excelente", "btn_active":"btn4"}); st.rerun()
                if b6.button(f"🔴 {mal_c} Mal", key="btn6"): st.session_state.update({"f_tipo":"CSI","f_val":"Malo", "btn_active":"btn6"}); st.rerun()
    ...
    # --- TAB 4: RECLAMOS (GRÁFICO MODERNO + TARJETAS POR INTENCIÓN) ---
    with tab4:
        ...
        if len(df_mes) > 0:
            ...
            with col_izq:
                ...
                temas = {"Sala/Espera": ["sala", "espera"], "Demora/Tiempo": ["demora", "tardó", "tarde"], "Limpieza": ["sucio", "lavado", "limpieza"], "Técnico": ["color", "diferencia", "alineado", "revisar"]}
                ...
                if df_frec['Casos'].sum() > 0:
                    # Gráfico Moderno con Amarillos
                    fig_b = px.bar(df_frec, x='Casos', y='Tema', orientation='h', color='Casos', color_continuous_scale=["#FFEC8B", "#FFD700", "#DAA520"])
                    fig_b.update_layout(showlegend=False, coloraxis_showscale=False, height=250, margin=dict(t=0,b=0,l=0,r=10),
                                        xaxis=dict(showgrid=True, gridcolor='rgba(0,0,0,0.1)', zeroline=True, zerolinecolor='rgba(0,0,0,0.5)'),
                                        yaxis=dict(showgrid=False))
                    fig_b.update_traces(marker_line_width=0, width=0.5); st.plotly_chart(fig_b, use_container_width=True)
            ...
            with col_der:
                ...
                for _, row in df_t.iterrows():
                    ...
                    st.markdown(f"""<div class="comentario-card {cls}"><div class="comentario-header">{row[col_cliente]} | {row['Intención']}</div><div class="comentario-body">{row[col_t_concatenado]}</div></div>""", unsafe_allow_html=True)
else:
