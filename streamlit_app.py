...
if df_raw is not None:
    ...
    # Comentarios específicos (Columnas I, M, O)
    col_coment_atencion = df.columns[8]  # I
    col_coment_calidad = df.columns[12]  # M
    col_coment_tiempo = df.columns[14]   # O
    col_coment_final = df.columns[17]    # R (Experiencia General)
    ...
    if len(df) > 0:
        ...
        with c2:
            st.plotly_chart(go.Figure(go.Indicator(mode="gauge+number", value=csi_val, title={'text': "CSI Unificado"}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "black"}, 'steps': [{'range': [0, 70], 'color': "#FF4B4B"}, {'range': [70, 85], 'color': "#FFA500"}, {'range': [85, 100], 'color': "#00CC96"}]})), use_container_width=True)
        ...
        if st.session_state.f_tipo and st.session_state.f_val:
            ...
            if col_busqueda in df.columns:
                ...
                # Definimos las columnas a mostrar según tu pedido
                cols_v = [
                    col_cliente, 
                    col_asesor, 
                    col_coment_atencion, # Comentario de H
                    col_coment_calidad,  # Comentario de L
                    col_coment_tiempo,   # Comentario de N
                    col_coment_final     # Comentario General
                ]
                # Filtramos solo las que existen
                cols_v = [c for c in cols_v if c in df.columns]
                
                st.subheader(f"Comentarios detallados - {st.session_state.f_tipo}: {st.session_state.f_val}")
                st.dataframe(df_final[cols_v], use_container_width=True)
        else:
            st.info("💡 Haz clic en un botón para ver los comentarios detallados de cada cliente.")
    else:
