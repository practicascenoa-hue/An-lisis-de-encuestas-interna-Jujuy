...
st.markdown("""
    ...
    /* Ajuste de margen inferior del gráfico para los botones */
...
if df_raw is not None:
    ...
    if len(df) > 0:
        ...
        # --- FUNCIÓN DE RELOJ MEJORADA ---
        def crear_gauge_moderno(valor, titulo):
            fig = go.Figure(go.Indicator(
                mode="gauge+number",
                value=round(valor, 1),
                title={'text': f"<b>{titulo}</b>", 'font': {'size': 20, 'color': '#2c3e50'}},
                number={'font': {'size': 50, 'color': '#2c3e50'}, 'suffix': "%" if "CSI" in titulo else ""},
                gauge={
                    'axis': {'range': [0, 100], 'tickwidth': 1, 'tickcolor': "#dee2e6"},
                    'bar': {'color': "#34495e", 'thickness': 0.2}, # Barra gris azulado profesional
                    'bgcolor': "white",
                    'borderwidth': 0,
                    'steps': [
                        {'range': [0, 60], 'color': "#f8d7da"},   # Rojo pastel
                        {'range': [60, 90], 'color': "#fff3cd"},  # Amarillo pastel
                        {'range': [90, 100], 'color': "#d1e7dd"}  # Verde pastel
                    ]
                }
            ))
            fig.update_layout(
        ...
        with col_main_1:
            ...
            # Alineación corregida con columnas de aire
