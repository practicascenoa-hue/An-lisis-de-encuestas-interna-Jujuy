...
if df_raw is not None:
    ...
    # Limpieza global (Asegura Tab 3 y Tab 4)
    ...
    # Refrescar df_anio y df_mes con datos limpios
    df_anio = df_raw[df_raw['Año'] == anio_sel].copy()
    df_mes = df_anio[df_anio['Mes_Num'] == mes_sel_num].copy()
    ...
    # --- TAB 1: INDICADORES (RECUPERADA COMPLETA) ---
    with tab1:
        if len(df_mes) > 0:
    ...
    # --- TAB 2: ASESORES (RECUPERADA COMPLETA) ---
    with tab2:
        ...
        if len(df_mes) > 0:
            ...
            with cb:
                ...
                df_final = df_res[[col_asesor, 'Total_Encuestas', 'Recibio_Seg_Count', '% Cumplimiento']]
                df_final.columns = ['Asesor', 'TOTAL ENCUESTAS', 'CON SEGUIMIENTO', '% Cumplimiento']
                st.dataframe(df_final.sort_values('TOTAL ENCUESTAS', ascending=False), use_container_width=True, hide_index=True)

    # --- TAB 3: EVOLUCIÓN (RECUPERADA COMPLETA) ---
    with tab3:
    ...
    # --- TAB 4: RECLAMOS (LA NUEVA VERSIÓN EXPANDIDA) ---
    with tab4:
        st.header("⚠️ Análisis de Calidad: Reclamos vs Promotores")
        if len(df_mes) > 0:
            def clasificar_intencion(row):
                nota = row[col_nps_puntaje]
                texto_raw = str(row[col_t_concatenado]).lower()
                frase_sistema = "sí, fue entregado en la fecha acordada ✔️"
                limpio = texto_raw.replace("-", "").replace("nan", "").strip()
                
                if nota <= 6: return "⚠️ Reclamo Crítico"
                elif nota >= 9:
                    if limpio == frase_sistema.lower() or limpio == "" or limpio == "sí":
                        return "✅ CONFORME"
                    else: return "💡 OPORTUNIDAD DE MEJORA"
                return "Neutral"
            ...
            with col_izq:
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("🟢 PROMOTORES", key="t4_p_f", type="primary" if st.session_state.tab4_filter == "Promotor" else "secondary"):
                        st.session_state.tab4_filter = "Promotor"; st.rerun()
                    st.metric("", cp)
                with c2:
                    if st.button("🔴 RECLAMOS", key="t4_r_f", type="primary" if st.session_state.tab4_filter == "Reclamo" else "secondary"):
